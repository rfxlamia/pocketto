...
function upsertGitattributes(existing, attrs) {
	const normalized = (existing || "").replace(/\r\n/g, "\n");
	const attrsBody = attrs.trimEnd();
	const marker = attrsBody.split("\n")[0]; // first line is the marker comment

	// Already contains our section — leave as-is (idempotent)
	if (normalized.includes(marker))
		return normalized.endsWith("\n") ? normalized : `${normalized}\n`;

	// Empty or missing — write fresh
	if (normalized.trim() === "") return attrs;

	// Merge: append Pocket section after existing content
	const prefix = normalized.endsWith("\n") ? normalized : `${normalized}\n`;
	return `${prefix}\n${attrs}`;
}

function buildEnterpriseBlock({
	enterprise,
	branch_strategy,
	create_pr,
	require_approval,
...
function runInit(
	targetDir,
	{ enterprise, branchStrategy, createPr, requireApproval, file },
) {
	const dir = path.resolve(targetDir || process.cwd());
	const config = validateInitFields({
		enterprise,
		branchStrategy,
		createPr,
		requireApproval,
	...
	if (!remoteUrl) {
		throw new CliError(
			...
			{
				human:
					"Error: No git remote configured. Add a remote (e.g. origin) and retry.",
			},
		);
	}
	...
	const existing = fs.existsSync(modePath)
		? fs.readFileSync(modePath, "utf8")
		: "";
	const existingAttrs = fs.existsSync(attrsPath)
		? fs.readFileSync(attrsPath, "utf8")
		: "";
	...
	fs.writeFileSync(
		modePath,
		upsertEnterpriseHeading(existing, block, config.file),
		"utf8",
	);
	fs.writeFileSync(
		attrsPath,
		upsertGitattributes(existingAttrs, GITATTRIBUTES),
		"utf8",
	);
