...
- `pocket-planning`, after you approve the plan, validates it with `structure --dry-run` and routes all plans to `pocket-structuring`.
- `pocket-structuring` decomposes all plans into an execution index (`execution-plan/index.md`) + per-task files (`execution-plan/tasks/T*-*.md`), generating phase manifests (`execution-plan/phase-N.md`) if multi-phase, and hands off phases one at a time.
